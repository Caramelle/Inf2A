					...
					if (exp.isAPP())
					{
						...
						if(e1.isArrow())
						{
							if(e1.left().equals(e2))
								return e1.right();
							else throw new TypeError("Function argument does not match expected type");
						}
						else throw new TypeError("Function type should be of arrow type");
						...
						if (exp.isINFIX())
						{
							...
							if(e1.isInteger() && e2.isInteger())
								 	if (exp.infixOp() == "==" || exp.infixOp() == "<=") return BoolType;
								 	else
								 		if(exp.infixOp() == "+" || exp.infixOp() == "-") return IntegerType;
								 		else throw new TypeError ("Operator not recognised");
							else
								throw new TypeError("One or both of the expressions are not of type Integer");
						}
						else
							if (exp.isIF())
							{
								...
								if(e1.isBool())
								{
									if(e2.equals(e3))
										return e2;
									else throw new TypeError("Types of the Then and Else statements do not match");
								}
							}
							else throw new TypeError("Type of if condition is not Boolean");
							return null;
